...
# MySQL 라이브러리 설정 - PyMySQL 우선 사용
MYSQL_LIBRARY = "pymysql"  # PyMySQL을 기본값으로 설정

# PyMySQL이 설치되어 있는지 확인하고 사용 가능하게 설정
try:
    import pymysql
...
if not MYSQL_CONNECTOR_AVAILABLE and not PYMYSQL_AVAILABLE:
    st.error("MySQL 라이브러리가 설치되지 않았습니다. 'pip install mysql-connector-python' 또는 'pip install pymysql'을 실행하세요.")
    st.stop()

# PyMySQL이 없으면 설치 안내
if not PYMYSQL_AVAILABLE:
    st.warning("PyMySQL이 설치되지 않았습니다. 더 안정적인 연결을 위해 'pip install pymysql'을 실행하세요.")

# DB 연결 설정 (MariaDB) - 단순화된 버전
DB_CONFIG = {
    "host": "localhost",
    "port": 3306,
    "user": "root",
...
# 사이드바 메뉴
st.sidebar.title("📊 Fund Returns 시스템")
...
if menu == "📤 데이터 업로드":
        ...
        if st.button("데이터 저장하기"):
            ...
            try:
                ...
                for attempt in range(max_retries):
                    try:
                        st.info(f"DB 연결 시도 {attempt + 1}/{max_retries}...")
                        
                        # 연결 설정 확인
                        st.info(f"연결 설정: {DB_CONFIG['host']}:{DB_CONFIG['port']}, DB: {DB_CONFIG['database']}")
                        
                        # 연결 생성 (단계별로 분리)
                        st.info("연결 객체 생성 중...")
                        
                        # 연결 설정 디버깅
                        st.info(f"연결 설정 상세: {DB_CONFIG}")
                        
                        # MySQL Connector 버전 확인
                        st.info(f"MySQL Connector 버전: {mysql.connector.__version__}")
                        
                        # 시스템 정보 확인
                        import platform
                        st.info(f"시스템: {platform.system()} {platform.release()}")
                        
                        # 메모리 사용량 확인 (안전한 방식)
                        try:
                            import psutil
                            memory = psutil.virtual_memory()
                            st.info(f"메모리 사용량: {memory.percent}% ({memory.available // 1024 // 1024}MB 사용 가능)")
                        except ImportError:
                            st.info("psutil 라이브러리가 설치되지 않음 - 메모리 정보 확인 불가")
                        except Exception as mem_error:
                            st.info(f"메모리 정보 확인 실패: {mem_error}")
                        
                        # 연결 시도 (PyMySQL 우선 사용)
                        conn = None
                        
                        # 방법 1: PyMySQL 시도 (우선)
                        if PYMYSQL_AVAILABLE and conn is None:
                            try:
                                st.info("방법 1: PyMySQL 시도...")
                                import pymysql
                                conn = pymysql.connect(**DB_CONFIG)
                                st.success("PyMySQL 연결 성공!")
                                MYSQL_LIBRARY = "pymysql"
                                
                            except Exception as e:
                                st.warning(f"PyMySQL 연결 실패: {e}")
                                conn = None
                        
                        # 방법 2: MySQL Connector 시도 (백업)
                        if MYSQL_CONNECTOR_AVAILABLE and conn is None:
                            try:
                                st.info("방법 2: MySQL Connector 시도...")
                                import mysql.connector
                                ...
                                st.success("MySQL Connector 연결 성공!")
                                
                                # 데이터베이스 선택
                                cursor_temp = conn.cursor()
                                cursor_temp.execute(f"USE {DB_CONFIG['database']}")
                                cursor_temp.close()
                                MYSQL_LIBRARY = "mysql.connector"
                                
                            except Exception as e:
                                st.warning(f"MySQL Connector 연결 실패: {e}")
                                conn = None
                        
                        # 모든 방법 실패
                        if conn is None:
                            st.error("모든 연결 방법 실패")
                            st.stop()
                        
                        st.info(f"최종 사용 라이브러리: {MYSQL_LIBRARY}")
                        ...
                        st.warning(f"데이터베이스 오류 (시도 {attempt + 1}): {mysql_error}")
                        ...
                        if attempt == max_retries - 1:
                            st.error("모든 연결 시도 실패. MariaDB 서비스가 실행 중인지 확인해주세요.")
                            st.stop()
                        else:
                            import time
                            st.info("2초 대기 후 재시도...")
                        ...
                        st.error(f"예상치 못한 오류 (시도 {attempt + 1}): {general_error}")
                        st.code(traceback.format_exc())
                        
                        if attempt == max_retries - 1:
                            st.error("연결 실패. 프로그램을 종료합니다.")
                            st.stop()
                        else:
                            import time
                            st.info("2초 대기 후 재시도...")
                ...
                try:
                    # 단순한 INSERT 쿼리 (ON DUPLICATE KEY UPDATE 제거)
                    insert_sql = f"""
                    ...
                    # 저장 확인
                    cursor.execute(f"SELECT COUNT(*) FROM {TABLE_NAME} WHERE asof_date = %s", (asof_date_str,))
        ...
        try:
            ...
            if conn:
                query = f"""
                    ...
                    FROM {TABLE_NAME}
                    WHERE asof_date BETWEEN '{start_date}' AND '{end_date}'
                ...
                if not df_analysis.empty:
                    ...
                    if show_histogram:
                        ...
                        selected_period = st.selectbox("히스토그램 분석 기간 선택", analysis_periods, key="histogram_period_select")
                    ...
                    rank_period = st.selectbox("순위 분석 기간 선택", analysis_periods, key="rank_period_select")
    ...
    if st.button("🏢 운용사별 분석 실행", type="primary"):
        try:
            ...
            if conn:
                ...
                df_manager = pd.read_sql(query, conn)
                conn.close()
    ...
    try:
        ...
        if conn:
            ...
            df_managers = pd.read_sql(manager_query, conn)
            conn.close()
                ...
                try:
                    ...
                    if conn:
                        query = f"""
                            ...
                            FROM {TABLE_NAME}
                            WHERE manager = '{selected_manager}'
                        ...
                        if not df_products.empty:
                            ...
                            if show_heatmap:
                                ...
                                try:
                                    ...
                                    # 히트맵 데이터 준비
                                    df_heatmap_long = df_heatmap.reset_index()
                                    df_heatmap_long['product_name'] = df_products_sorted['product_name']
                                    df_heatmap_long = df_heatmap_long.melt(
                                        id_vars=['product_name'], 
                                        value_vars=df_heatmap.columns,
                                        var_name='수익률_기간', 
                                        value_name='수익률'
                                    )
            ...
            if conn:
                query = f"""
                    SELECT asof_date, 
                           ...
                           SUM(total_amount) as total_assets
                    FROM {TABLE_NAME}
                    WHERE asof_date BETWEEN '{analysis_start}' AND '{analysis_end}'
                    GROUP BY asof_date
                    ORDER BY asof_date
                """
    ...
    try:
        ...
        if conn:
            ...
            manager_query = f"""
                ...
                FROM {TABLE_NAME} 
                WHERE manager IS NOT NULL 
                AND asof_date BETWEEN '{timeline_start}' AND '{timeline_end}'
                ORDER BY manager
            """
            df_managers = pd.read_sql(manager_query, conn)
            conn.close()
